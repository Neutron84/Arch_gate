...
cleanup_vnc_files() {
    ...
    if [ -d "$HOME/.vnc" ]; then
        rm -f "$HOME/.vnc/localhost:"*.log > /dev/null 2>&1
        rm -f "$HOME/.vnc/"*.log > /dev/null 2>&1
    fi

    # Clean up X lock files (X1-lock, X2-lock, etc.)
    if [ -d "$PREFIX/tmp" ]; then
        rm -f "$PREFIX/tmp/.X"*"-lock" > /dev/null 2>&1
    fi

    # Clean up X11 socket files (X1, X2, etc.)
    if [ -d "$PREFIX/tmp/.X11-unix" ]; then
        rm -f "$PREFIX/tmp/.X11-unix/X"* > /dev/null 2>&1
    fi
}
...
graceful_stop() {
    ...
    if [ -n "$display_numbers" ]; then
        ...
        if [ -d "$PREFIX/tmp" ]; then
            lock_files=$(find "$PREFIX/tmp" -name ".X*-lock" -type f 2>/dev/null | sed 's/.*\.X\([0-9]*\)-.*/:\1/' | sort -u)
